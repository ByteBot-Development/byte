...
const ban = {
	data: new SlashCommandBuilder()
		...
		.addUserOption((user) =>
			user.setName(`member`).setDescription(`The member you want to ban`).setRequired(true)
		)
		.addStringOption((reason) =>
			reason.setName(`reason`).setDescription(`The reason you want to ban the user for!`)
		)
			...
			return await interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setTitle(`Failed`)
						.setDescription(
							`You cannot ban this user because your role's highest position ${
			...
			return await interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setTitle(`Failed`)
						.setDescription(
							`I cannot ban this user because my role's highest position ${
			...
			await interaction.channel.send({
				embeds: [
					new EmbedBuilder()
						.setTitle(`Failed`)
						.setDescription(
							`Couldnt ban ${member}! Make sure my role has the \`BAN MEMBERS\` permission.`
						)
						.setColor(`RED`)
						.setFooter({
							text: interaction.user.tag,
